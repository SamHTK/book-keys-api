...
module.exports = async function (context, req) { try { const fromUpn = process.env.REQUEST_FROM_UPN || process.env.SCHEDULER_UPN; if (!fromUpn) throw new Error("Missing REQUEST_FROM_UPN or SCHEDULER_UPN app setting.");
...
if (!execEmail || !start || !end || !custName || !custEmail) {
  context.res = { status: 400, body: { error: "Missing required fields: execEmail, start, end, customer{name,email}" } };
  return;
}
